...
- Add a **worker process** (`src/worker.ts`, BullMQ worker) that consumes jobs off the queue and applies `UPDATE songs SET listens = listens + $delta WHERE id = $songId` — the worker's only job is this Postgres write; the Redis key was already atomically cleared by the scheduler's `take()` (`GETDEL`) before the job was ever enqueued, so there's nothing Redis-side left for the worker to do
